...
if not exist "%PROJECT_DIR%\package.json" (
    ...
    echo Put START_VKarmani.bat in the VKarmani-Desktop folder.
    echo.
    pause
    exit /b 1
)
...
where node >nul 2>nul || (
    ...
    echo Install Node.js 22 LTS, reopen CMD/PowerShell and run this BAT again.
    echo.
    pause
    exit /b 1
)
...
where npm >nul 2>nul || (
    ...
    echo Reinstall Node.js 22 LTS with npm enabled.
    echo.
    pause
    exit /b 1
)
...
call :ensure_pkg "@vitejs/plugin-react/package.json" "npm install -D @vitejs/plugin-react@4.3.4 --no-audit --fund=false" "@vitejs/plugin-react"
if errorlevel 1 goto :npm_failed
...
if not exist "node_modules\@tauri-apps\cli\tauri.js" (
    echo [INFO] Tauri CLI executable is missing. Repairing...
    call npm install -D @tauri-apps/cli@2.10.1 --no-audit --fund=false --registry=https://registry.npmjs.org/
    if errorlevel 1 goto :npm_failed
    set "REPAIRED=1"
)
...
:run_tauri
set "TAURI_JS=%PROJECT_DIR%\node_modules\@tauri-apps\cli\tauri.js"
if not exist "%TAURI_JS%" (
    echo [ERROR] Tauri CLI was not installed correctly:
    echo %TAURI_JS%
    echo Run npm install in the project folder and try again.
    echo.
    exit /b 1
)
call node "%TAURI_JS%" %*
